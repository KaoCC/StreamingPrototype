...
namespace SP {
	...
	size_t Packet::decodeHeader(const DataBuffer & buffer) const {
		...
		for (size_t i = 0; i < HEADER_SIZE; ++i) {
			...
			msgSize = msgSize * 256 + static_cast<size_t>(buffer[HEADER_SIZE - 1 - i] & 0xFF);
		}
	...
	void Packet::encodeHeader(DataBuffer & buffer, size_t size) const {
		...
		// KAOCC: check the endianness
		buffer[3] = static_cast<uint8_t>((size >> 24) & 0xFF);
		buffer[2] = static_cast<uint8_t>((size >> 16) & 0xFF);
		buffer[1] = static_cast<uint8_t>((size >> 8) & 0xFF);
		buffer[0] = static_cast<uint8_t>((size) & 0xFF);
	}
